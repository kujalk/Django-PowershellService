...
def firewall_status(request):
    if request.method == "GET":
        ...
        if firewall_serializer.is_valid():
            ...
            return JsonResponse(
                firewall_serializer.data, status=status.HTTP_201_CREATED, safe=False
            )

        return JsonResponse(
            firewall_serializer.errors, status=status.HTTP_400_BAD_REQUEST, safe=False
        )
        ...
        return JsonResponse(
            {
                "message": "{} Service Status were deleted successfully!".format(
                    count[0]
                )
            },
...
def firewall_service(request, pk):
    try:
        ...
        return JsonResponse(
            {"message": "The Service Status does not exist"},
    ...
    if request.method == "GET":
        ...
        if firewall_serializer.is_valid():
            ...
            return JsonResponse(firewall_serializer.data)
        return JsonResponse(
            firewall_serializer.errors, status=status.HTTP_400_BAD_REQUEST
        )

    elif request.method == "DELETE":
        service.delete()
        return JsonResponse(
            {"message": "Service Status was deleted successfully!"},
            status=status.HTTP_204_NO_CONTENT,
        )
